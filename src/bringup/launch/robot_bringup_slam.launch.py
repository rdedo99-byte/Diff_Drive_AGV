...
def generate_launch_description():
    ...
    rviz_config_file = PathJoinSubstitution([bringup_pkg_share, 'config', 'rviz', 'rviz.rviz'])
    controller_yaml = PathJoinSubstitution([bringup_pkg_share, 'config', 'ros2_control', 'diff_drive_controller.yaml'])
    slam_params_file = PathJoinSubstitution([bringup_pkg_share, 'config', 'slam', 'slam_params.yaml'])
    ...
    slam_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([slam_toolbox_pkg_share, 'launch', 'online_async_launch.py'])
        ]),
        launch_arguments={
    ...
    return LaunchDescription([
        ...
        # 4. SLAM 
        TimerAction(period=11.0, actions=[slam_launch]),
        # 5. Navigation
        TimerAction(period=16.0, actions=[nav2_launch]),
    ])